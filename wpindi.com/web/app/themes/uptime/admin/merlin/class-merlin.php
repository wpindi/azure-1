...
class Merlin {
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'uptime' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return esc_html__( 'Please define default parameters in the form of an array.', 'uptime' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return esc_html__( 'Please define an SVG icon filename.', 'uptime' );
		}
	...
	public function steps() {
		...
		$this->steps = array(
			'welcome' => array(
				'name'    => esc_html__( 'Welcome', 'uptime' ),
		...
		if ( class_exists( 'TGM_Plugin_Activation' ) ) {
			$this->steps['plugins'] = array(
				'name' => esc_html__( 'Plugins', 'uptime' ),
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'uptime' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'uptime' ),
	...
	<?php
		$this->logger->debug( esc_html__( 'The welcome step has been displayed', 'uptime' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( esc_html__( 'The license activation step has been displayed', 'uptime' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The child theme installation step has been displayed', 'uptime' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'uptime' ); ?>">
										<?php esc_html_e( 'req', 'uptime' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( esc_html__( 'The plugin installation step has been displayed', 'uptime' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				<p><?php esc_html_e( 'Select which demo data you want to import:', 'uptime' ); ?></p>
	...
	<?php
		$this->logger->debug( esc_html__( 'The content import step has been displayed', 'uptime' ) );
	}
	...
	<?php
		$this->logger->debug( esc_html__( 'The final step has been displayed', 'uptime' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( esc_html__( 'The existing child theme was activated', 'uptime' ) );
		...
		$this->logger->debug( esc_html__( 'The newly generated child theme was activated', 'uptime' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'uptime' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'uptime' ),
				)
			);
		}
		...
		$this->logger->debug( esc_html__( 'The license activation was performed with the following results', 'uptime' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'uptime' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'uptime' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'uptime' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'uptime' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'uptime' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'uptime' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'uptime' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( esc_html__( 'The child theme functions.php content was generated', 'uptime' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$output = "
			...
			*/\n
			...
			@import url('../{$slug}/style.css');
		";

		// Let's remove the tabs so that it displays nicely.
		$output = trim( preg_replace( '/\t+/', '', $output ) );

		$this->logger->debug( esc_html__( 'The child theme style.css content was generated', 'uptime' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( esc_html__( 'The child theme screenshot was copied to the child theme, with the following result', 'uptime' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( esc_html__( 'The child theme screenshot was not generated, because of these results', 'uptime' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'uptime' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'uptime' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'uptime' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'uptime' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'uptime' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'uptime' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'uptime' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call failed to start, because of incorrect data', 'uptime' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'uptime' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'uptime' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'uptime' ),
				array(
			...
			wp_send_json(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Error', 'uptime' ),
					'logs'    => '',
					'errors'  => '',
				)
			);
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( esc_html__( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'uptime' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'uptime' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'uptime' ),
				'description'      => esc_html__( 'Demo content data.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'uptime' ),
				'description'      => esc_html__( 'Sample widgets data.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'uptime' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'uptime' ),
				'description'      => esc_html__( 'Sample theme options data.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'uptime' ),
				'description'      => esc_html__( 'Redux framework options.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'uptime' ),
				'description'      => esc_html__( 'After import setup.', 'uptime' ),
				'pending'          => esc_html__( 'Pending', 'uptime' ),
				'installing'       => esc_html__( 'Installing', 'uptime' ),
				'success'          => esc_html__( 'Success', 'uptime' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( esc_html__( 'The revolution slider import was executed', 'uptime' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'uptime' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( esc_html__( 'The home page was set', 'uptime' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( esc_html__( 'The blog page was set', 'uptime' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( esc_html__( 'The Hello world post status was set to draft', 'uptime' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( esc_html__( 'This predefined demo import does not have the name parameter: import_file_name', 'uptime' ), $import_file );
			}
		}
